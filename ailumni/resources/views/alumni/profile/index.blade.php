...
<div>
    <div class="max-w-10rem mx-auto py-10 sm:px-6 lg:px-8">
        ...
        <div class="mx-auto" style="width: 70rem; display: flex; justify-content: space-between; align-items: center;">
            <form method="GET" action="{{ route('alumni.profile.index') }}" class="flex" style="width: 60rem;">
            ...
            @if($userCanViewFullInfo)
                <button id="toggleInfo" class="bg-green-500 text-white rounded-md px-4 py-2">
                    ...
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg mx-auto" style="width: 80rem;">
                        <table class="min-w-full divide-y divide-gray-200 w-full" style="table-layout: fixed;">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200" id="alumniTableBody">
                                @include('alumni.partials.table_rows', ['alumni' => $alumni, 'showFullInfo' => $showFullInfo])
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    <script>
        ...
        function loadMoreAlumni() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    showFullInfo: {{ $showFullInfo ? 'true' : 'false' }}
                },
        ...
        $('#toggleInfo').on('click', function() {
            const showFullInfo = $(this).text().includes('Show');
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('#alumniTableBody').html(data);
                    $('#toggleInfo').text(showFullInfo ? 'Hide Full Info' : 'Show Full Info');
                }
            });
        });
    </script>
